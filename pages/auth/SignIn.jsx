...
const SignIn = ({}) => {
    ...
    const onSubmitFormik = async ({ email }) => {
        ...
        setStatus(
            'If an account is linked to this address, you will receive an email to reset your password.',
        )

        if (isNotUser.result === true) {
            return
        } // if no user has been found STOP

        if (isNotUser.provider !== 'credentials') {
            return
        } // if user uses oAuth STOP

        const sendPwdRecover = await sendPwdResetMail(email)
        if (!sendPwdRecover.success) {
            setStatus('An error occured')
            return
        } // if error in sending email occured STOP
    }
    ...
    const validStyling = () => {
        if (formik.errors.email && formik.touched.email) {
            return {
                ...
                message: <span className="text-red-600">{formik.errors.email}</span>,
            }
        } else {
    ...
    if (!isForgotPasswordMode) {
        ...
        return (
            <>
                <section className="bg-gray-50 mt-2 flex items-center justify-center gap-x-12">
                    <div className="bg-gray-100 flex-ROW rounded-2xl shadow-lg max-w-md p-5 items-center">
                        {/* TEXT */}
                        <div className=" ">
                            ...
                            {formik.submitCount === 0 ? (
                                <div>
                                    <p className="text-xs mt-4 text-[#002D74]">
                                        ...
                                        account and we will send you a link to reset your
                                        password.{' '}
                                    </p>
                                </div>
                            ) : (
