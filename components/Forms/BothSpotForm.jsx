...
const BothSpotForm = ({ onAddOrEditFx, previousValues }) => {
  ...
  const validationSchemaYup = Yup.object().shape({
    title: Yup.string()
      ...
      .min(
        6,
        `The title should be more than 6 characters, type ${
          5 - characterCountTitle
        } more!`
      )
  ...
  const onSubmitFormik = async (formValues) => {
    ...
    // Adding region
    const found = countryContinent.find(
      ...
      >
        <div>
          <InputsBoth
            ...
            formikName="title"
            formikHasFieldBeenTouched={
              formik.touched.title == undefined ? false : formik.touched.title
            }
        ...
        <div className="mb-6">
          ...
          <span className="text-red-600 block">{categoryErrorMsg}</span>
        </div>
